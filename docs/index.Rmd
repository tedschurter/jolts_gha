...
# import latest JOLTS data from BLS API
...
# write newest data to csv file
write_csv(jolts, "../clean_data/new_jolts.csv")

```


```{r, import existing jolts data}

# import existing JOLTS data from AWS S3 storage 

# assign bucket, and region
...
old_jolts <- read_csv(
  # use save object function from aws.s3 package
  save_object(
  object = "current_jolts.csv", # object name in S3 bucket
  bucket = bucket,
  region = region,
  file = "../clean_data/current_jolts_20231009.csv" ) # local location and name for file 
  ) 
...
```{r, compare existing and new jolts data, include=F}

# if the data from BLS (jolts dataframe) is the same as the existing data in the AWS bucket (old_jolts dataframe), no update is needed, stop running the Rmd file; no index will be created.

 if(identical(jolts[1,4], old_jolts[1,4]) == F) stop("No update needed")
("")

# if the data from BLS (jolts dataframe) is not the same as the existing data in the AWS bucket (old_jolts dataframe), replace the file in the AWS bucket with the most recent data from BLS

# if(identical(jolts[1,4], old_jolts[1,4]) == F)  put_object(
#   file = "clean_data/new_jolts.csv", 
#   object = "current_jolts.csv", 
#   bucket = bucket,
#   region = region
# )
